...
\cmd{!suena \param{N}} & Anuncia la petición número $N$ como ``Está sonando\ldots'' en todos los canales y la elimina de la cola. \textcolor{red}{Este comando ayuda tanto a los DJs como a los oyentes. Para los DJs el bot se da por enterado de que esa petición ya se ha emitido y la borra de la lista de peticiones pendientes, y para los oyentes, saben qué está sonando y por qué petición se va emitiendo, por lo que pueden saber cuánto falta para la suya.} \\[0.3em]
...
\textbf{Comando} & \textbf{Descripción} \\
\midrule
\bot{} \cmd{sal del canal} & El bot abandona el canal actual. Es temporal: se re-une en el siguiente chequeo peri\'{o}dico o PING. \\[0.3em]
...
\subsection{Detección del nick del bot}

El bot puede funcionar en dos modos de detección de su nick:

\begin{itemize}[leftmargin=2em]
  \item \textbf{Modo estricto} (por defecto): solo responde si el mensaje contiene exactamente su nick (ej.\ \texttt{c3pO}).
  \item \textbf{Modo relajado}: responde también a variaciones del nick y a alias configurados. Por ejemplo, \texttt{c3po}, \texttt{C3PO} o un alias como \texttt{cetrés} podrían activar al bot.
\end{itemize}

El modo activo lo configura un administrador con \cmd{!nickmode}. Si notas que el bot responde a mensajes que no iban dirigidos a él, o al contrario, que no responde cuando lo mencionas, puede deberse a este ajuste.
